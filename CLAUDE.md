...
1. **Public ingest API** (`src/public-api/`, mounted at `/api/v1`): plain REST, authed by API key (`Bearer` token verified with `auth.api.verifyApiKey`). This is what the reporter/CLI hit. Kept REST so any CI/curl/language can post. The token's `referenceId` is the owning user id.
