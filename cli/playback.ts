...
import { ingestPlaywrightReport } from '../src/lib/normalize'
import {
  manifestSchema,
  SCHEMA_VERSION,
  type Manifest,
...
async function main(): Promise<void> {
  ...
  if (positionals.length !== 1) fail('pass exactly one Playwright results.json path')
  if (!values.project) fail('--project <id> is required')

  const reportFile = positionals[0]
  if (!existsSync(reportFile)) fail(`report not found: ${reportFile}`)
