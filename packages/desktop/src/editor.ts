...
// GUI-launched apps inherit a stripped PATH, so the editor CLI often isn't found;
// include the usual install dirs.
...
export function openInEditor(repoRoot: string, file: string, line: number, column: number): Promise<void> {
  ...
  const env = { ...process.env, PATH: [process.env.PATH, ...EXTRA_PATH].filter(Boolean).join(':') }
